...
### 5. Commit referenced context to the default branch first

A cold agent boots from only the ticket URL + a clean checkout — it never sees this conversation. So **any artifact a ticket references (spec, ADR, updated backlog) must be committed to the default branch before the ticket is filed.** An untracked spec is a dead link. Commit these first, then publish.
